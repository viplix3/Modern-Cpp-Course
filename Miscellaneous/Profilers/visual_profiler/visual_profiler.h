...
public:
    InstrumentationTimer(const std::string& name)
        : m_profResult({name, 0, 0, 0}), m_Stopped(false)
    {
    ...
    void Stop()
    {
        ...
        m_profResult.Start = std::chrono::time_point_cast<std::chrono::microseconds>(m_StartTimepoint).time_since_epoch().count();
        m_profResult.End = std::chrono::time_point_cast<std::chrono::microseconds>(endTimepoint).time_since_epoch().count();

        m_profResult.ThreadID = std::hash<std::thread::id>{}(std::this_thread::get_id());
        Instrumentor::Get().WriteProfile(m_profResult);
